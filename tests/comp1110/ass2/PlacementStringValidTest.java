...
public class PlacementStringValidTest {
    ...
    // u6923736
    @Test
    public void offBoardpieceA(){
        for (int i = 0; i < OFF_BOARD_A.length; i++){
            test(OFF_BOARD_A[i], "", false);
        }

        for (int j = 0; j < SOLUTIONS.length; j++){
            String goodPieceA = SOLUTIONS[j].placement.substring(0,4);
            test(goodPieceA, "", true);
        }
    }
    ...
    public void overlapC() {
        for (int i = 0; i < SOLUTIONS.length; i++) {
            ...
            for (int p = 0; p < 10; p++) {
                ...
                test(badPlacement, badPiece, false);
                System.out.println(badPlacement);
            }
            test(SOLUTIONS[i].placement, "", true);
        }
    }
